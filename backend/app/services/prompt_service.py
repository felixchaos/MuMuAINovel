...
**7a. 文本质感审计 (Text Quality Audit) - 只做诊断，不改写正文**
把下面维度纳入评分依据和 suggestions，避免只看剧情不看读感：
- AI味/模板味：检查是否存在总结式连接词、否定式排比、自问自答、抽象大词、模板化外貌/动作描写。
- 移动端阅读节奏：检查段落是否过长、句子是否一逗到底、长短句是否缺少呼吸、破折号是否过密。
- 时空与五感锚点：检查章节开篇或场景切换是否有时间/地点/天气/声音/触觉/气味等锚点。
- 对话潜台词：检查关键对话是否只是直说信息，人物说话方式是否有区分。
- 情节轻重：检查是否所有事件平均用力，高潮、转折、过渡是否有详略层次。
- 冲突立场：检查主要冲突是否有清晰的情绪立场，避免双方各打五十大板导致读者无爽点/痛点。
...
{{
  ...
  "quality_audit": {{
    "ai_flavor_level": "low|medium|high",
    "mobile_rhythm": "段落、句长、断句和破折号的读感诊断",
    "sensory_anchor": "时空锚点与五感是否足够，缺什么",
    "dialogue_subtext": "关键对话是否有潜台词、人物语气是否区分",
    "template_risks": ["发现的模板词/模板动作/模板句式；没有则为空数组"],
    "summary": "一句话总结文本质感最大问题或优势"
  }},
  "pacing": "varied",
  "dialogue_ratio": 0.4,
...
✅ quality_audit 必填：即使没有明显问题，也要给出 low 风险和简短说明
...
✅ 如果发现文本质感问题，建议类型优先使用：【AI味】【排版问题】【时空锚点】【对话潜台词】【情节轻重】【冲突立场】
...
""")
        ...
        # 职业/力量体系
        if project_context.get('chapter_careers'):
            prompt_parts.append(f"""## ⚔️ 职业/力量体系

{project_context['chapter_careers']}
